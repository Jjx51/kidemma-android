...
import com.kidemma.homeAdmin.tabs.agenda.presentation.components.AgendaTabUiConstants.Dimens.ClassTimeRowPadding
import com.kidemma.homeAdmin.tabs.agenda.presentation.components.AgendaTabUiConstants.Dimens.ClassesListContentPadding
import com.kidemma.homeAdmin.tabs.agenda.presentation.components.AgendaTabUiConstants.Dimens.ClassesListItemSpacing
...
private fun ClassCard(
    ...
    // Decided to keep logic here for simplicity,
    // but if it grows more complex we can move it to a ViewModel or helper function
    val maxKids = AgendaTabUiConstants.Numbers.MAX_KIDS_DISPLAYED
    val hasMoreKids = classItem.kids.size > maxKids
    val kidsToDisplay =
        if (!isExpanded && hasMoreKids) classItem.kids.take(maxKids) else classItem.kids
